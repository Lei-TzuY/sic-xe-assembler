...
from callsite_transfers import (
    MODULUS,
    _base_relative,
...
class SparseLinearTransferError(ValueError):
    pass
...
def _normalize(terms, offset=0):
    ...
    for register, coefficient in terms:
        ...
        if coefficient:
            merged[register] = _signed_coefficient(
                merged.get(register, 0) + coefficient
            )
            if merged[register] == 0:
                merged.pop(register, None)
    if len(merged) > MAX_LINEAR_TERMS:
        return None
...
def _source(register):
    return _normalize(((register, 1),), 0)


def _is_const(expr):
    return expr is not None and not expr[0]


def _scale_expr(expr, factor):
    if expr is None:
        return None
    factor = _signed_coefficient(factor)
    terms, offset = expr
    return _normalize(
        ((register, coefficient * factor) for register, coefficient in terms),
        offset * factor,
    )
...
def _sub_expr(left, right):
    return _add_expr(left, _scale_expr(right, -1))
...
def _mul_expr(left, right):
    if left is None or right is None:
        return None
    if _is_const(left):
        return _scale_expr(right, _signed24(left[1]))
    if _is_const(right):
        return _scale_expr(left, _signed24(right[1]))
    return None
...
def _deserialize(spec):
    ...
    if kind == "affine" and spec.get("source") in TRACKED_REGISTERS:
        return _normalize(
            ((spec["source"], spec.get("scale", 1)),),
            spec.get("offset", 0),
        )
    if kind == "linear":
        coefficients = spec.get("coefficients") or {}
        return _normalize(coefficients.items(), spec.get("offset", 0))
    return None


def _input_registers(spec):
    expression = _deserialize(spec)
    if expression is None:
        return ()
    return tuple(register for register, _ in expression[0])
    ...
    if mnemonic in ("ADD", "SUB", "MUL", "DIV", "AND", "OR"):
        ...
        if mnemonic == "ADD":
            ...
            if target & REGISTER_MASK == 0:
                ...
                state["A"] = _const(current[1] & target)
            else:
                state["A"] = None
        else:
            if target & REGISTER_MASK == 0:
...
def _substitute(expression, inputs):
    if expression is None:
        return None
    terms, offset = expression
    result = _const(offset)
    for register, coefficient in terms:
        source_expression = inputs.get(register)
        if source_expression is None:
            return None
        result = _add_expr(result, _scale_expr(source_expression, coefficient))
        if result is None:
            return None
    return result
...
def _apply_symbolic_summary(outgoing, summary):
    ...
    for register, spec in (summary.get("return_linear_transfers") or {}).items():
        if register not in TRACKED_REGISTERS:
            continue
        result[register] = _substitute(_deserialize(spec), inputs)
    return result
...
def _derive_summary(structural, outgoing):
    ...
    if returns and all(outgoing.get(site) is not None for site in returns):
        for register in TRACKED_REGISTERS:
            ...
            if not terms:
                continue
            if terms == ((register, 1),) and offset % MODULUS == 0:
                continue
            transfers[register] = _serialize(expression)
    summary["return_linear_transfers"] = transfers
    summary["linear_input_registers"] = sorted({
        register
        for spec in transfers.values()
        for register in _input_registers(spec)
    })
    summary["linear_return_registers"] = sorted(transfers)
    summary["multivariate_return_registers"] = sorted(
        register
        for register, spec in transfers.items()
...
def _merge_summary_layers(structural, register_summaries, transfer_hints):
    ...
    for entry, base in structural.items():
        ...
        merged["return_linear_transfers"] = dict(
            transfer.get("return_linear_transfers") or {}
        )
        result[entry] = merged
    return result
        ...
        (
            ...
            tuple(
                ...
                for register, spec in sorted(
                    summary.get("return_linear_transfers", {}).items()
                )
            ),
        )
...
def infer_sparse_linear_return_transfers(nodes, edges, register_summaries=None):
    """Infer bounded sparse multivariate linear register-return formulas."""
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        for entry, shape in structural.items():
            _, outgoing = _analyze_function_symbolically(
                nodes,
                edges,
                entry,
                available,
            )
            symbolic = _derive_summary(shape, outgoing)
            register = (register_summaries or {}).get(entry) or {}
            symbolic["return_constants"] = dict(
            ...
            symbolic["return_conditions"] = list(
                register.get("return_conditions") or ()
            )
        ...
        previous = signature
        hints = derived
    raise SparseLinearTransferError(
        "Sparse linear return-transfer inference did not converge"
    )
...
def _evaluate_range(spec, inputs):
    ...
    if expression is None:
        return None
    terms, offset = expression
    low = high = _signed24(offset)
    for register, coefficient in terms:
        interval = inputs.get(register)
        if interval is None:
            return None
        candidates = (
...
def _clear_owned_base(node):
    ...
    changed = (
        ...
        or node.get("target_resolution") in SPARSE_LINEAR_BASE_RESOLUTIONS
...
def _resolve_base_targets(nodes, exact_in, range_in):
    ...
    for node in nodes:
        ...
        if resolution is not None and resolution not in SPARSE_LINEAR_BASE_RESOLUTIONS:
            continue
        ...
        new_resolution = None
        if base is not None:
            new_resolution = "sparse-linear-base"
            ...
            if interval is not None and interval[0] == interval[1]:
                ...
                new_resolution = "sparse-linear-range-base"
        if base is None:
            if resolution in SPARSE_LINEAR_BASE_RESOLUTIONS:
    ...
    for edge in edges:
        ...
        if exact_state is not None and exact_state.get("CC") in CONDITION_VALUES:
            if not _exact_edge_feasible(node, edge, exact_state):
                ...
                edge["resolution"] = "sparse-linear-condition"
                continue
        ...
        if range_state is not None and range_state.get("CC") is not None:
            if not _range_edge_feasible(node, edge, range_state):
                ...
                edge["feasible"] = False
                edge["reason"] = "condition-false"
                edge["resolution"] = "sparse-linear-range-condition"
...
def _instantiations(nodes, summaries, exact_out, range_out):
    ...
    for node in nodes:
        ...
        for register, spec in (
            summary.get("return_linear_transfers") or {}
        ).items():
        ...
        item = {
            ...
            "exact": exact,
            "ranges": ranges,
            "transfers": dict(summary.get("return_linear_transfers") or {}),
        }
        node["sparse_linear_instantiation"] = item
...
def refine_sparse_linear_transfers(
    nodes,
    edges,
    entry_address,
    ...
    """Instantiate bounded multivariate linear summaries at concrete calls."""
    initial = {} if base_register is None else {"B": base_register}
    previous = None
    max_iterations = max(5, len(nodes) + 5)
    for iteration in range(1, max_iterations + 1):
        inferred = infer_sparse_linear_return_transfers(
            nodes,
            edges,
        ...
        for node in nodes:
            ...
            if node["base_mnemonic"] == "JSUB":
                node["sparse_linear_transfer_summary"] = summaries.get(
                    node.get("target")
                )
        ...
        if signature == previous:
            return {
                ...
                "base_resolutions": sum(
                    1
                    for node in nodes
                    if node.get("target_resolution") == "sparse-linear-base"
                ),
                "range_base_resolutions": sum(
                    1
                    for node in nodes
                    if node.get("target_resolution") == "sparse-linear-range-base"
                ),
                "multivariate_transfers": sum(
                    len(summary.get("multivariate_return_registers") or ())
                    for summary in summaries.values()
                ),
            }
        previous = signature
    raise SparseLinearTransferError(
        "Sparse linear transfer/CFG refinement did not converge"
    )
