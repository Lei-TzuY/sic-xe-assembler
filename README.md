...
A successful CLI link also writes a deterministic linker report beside the first object input (`program.obj` → `program.map`). The report contains CSECT layout, the final ESTAB, definition provenance, symbol cross-references, unused R declarations, exact relocation arithmetic, source/load relocation addresses, and entry-point provenance. Map generation is atomic, and a failed link removes any stale report from a previous run. See [`docs/link-map.md`](docs/link-map.md) for the stable report format.
